...
if getgenv == nil then 
	getgenv = function() return getfenv(0) end
end
...
local CoreGui = (RunService:IsStudio() and Players.LocalPlayer.PlayerGui or game:GetService("CoreGui"))

-- Interface Management
local Rayfield = (RunService:IsStudio() and script.Parent.RayfieldUI or game:GetObjects("rbxassetid://10804731440")[1])
	...
	function Window:CreateTab(Name,Image)
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			function DropdownSettings:Add(new_option) new_option = tostring(new_option)
				...
				DropdownOption.Title.TextTransparency = (Dropdown.List.Visible and 0 or 1)

				DropdownOption.Interact.ZIndex = 50
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					if Dropdown.Selected.Text ~= new_option then
						Dropdown.Selected.Text = new_option

						local Success, Response = pcall(function()
							DropdownSettings.Callback(new_option)
						end)
						if not Success then
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
							TweenService:Create(Dropdown.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 1}):Play()
						...
						DropdownSettings.CurrentOption = new_option
					...
					for _, Option in ipairs(DropdownSettings.Options) do
						...
						DropdownOption.Interact.MouseButton1Click:Connect(function()
							if Dropdown.Selected.Text ~= Option then
								...
								TweenService:Create(DropdownOption.UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {Transparency = (Dropdown.List.Visible and 0 or 1)}):Play()
								TweenService:Create(DropdownOption, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {BackgroundColor3 = Color3.fromRGB(40, 40, 40)}):Play()
								Debounce = true
								...
								for _, DropdownOpt in ipairs(Dropdown.List:GetChildren()) do
									if DropdownOpt.ClassName == "Frame" and DropdownOpt.Name ~= "Placeholder" then
										TweenService:Create(DropdownOpt, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {BackgroundTransparency = (Dropdown.List.Visible and 0 or 1)}):Play()
										TweenService:Create(DropdownOpt.UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {Transparency = (Dropdown.List.Visible and 0 or 1)}):Play()
										TweenService:Create(DropdownOpt.Title, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {TextTransparency = (Dropdown.List.Visible and 0 or 1)}):Play()
									end
								end
								
								TweenService:Create(Dropdown.List, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {ScrollBarImageTransparency = (Dropdown.List.Visible and 0 or 1)}):Play()
								TweenService:Create(Dropdown.Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Quint), {Rotation = 180}):Play()	
